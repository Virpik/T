...
public struct TManagedObjects<T: NSManagedObject> {
    ...
    public func entities() -> [T] {
        ...
        let result = try? self.find(with: request)
    ...
    public func find(params: [(String, Any)] = [], sortBy: [Sorting] = [], fetchLimit: Int? = nil) -> [T] {
        ...
        let result = try? self.find(predicate: predicate, sortDescriptors: descriptors, fetchLimit: fetchLimit)
        
        return result ?? []
    }
    
    public func find(predicate: NSPredicate?, sortDescriptors: [NSSortDescriptor] = [], fetchLimit: Int? = nil) throws ->  [T] {
    ...
    public func find(with request: NSFetchRequest<NSFetchRequestResult>) throws -> [T] {
        let context = self.coreDataManager.defaultContext
